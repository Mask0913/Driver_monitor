    ...
    def feedCap(self, frame, bboxes, size=720, face_size=20):
        ...
        if len(rects) or len(bboxes):
            if len(rects):
                ...
                areas = [v[-1] * v[-2] for v in bboxes]
                max_i = np.argmax(areas)
                left, top, right, bottom = bboxes[max_i]
                top = int(top * ratio)
        ...
        if self.TOTAL >= 5 and (self.mTOTAL >= 2 or self.hTOTAL >= 2):
            cv2.putText(frame, "SLEEP!!!", (100, 230),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3)
            if self.flag == 0:
                _thread.start_new_thread(playsound, ('Sound/exhausted.mp3',))
                self.flag = 1
